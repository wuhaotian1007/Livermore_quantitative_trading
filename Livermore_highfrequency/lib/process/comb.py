...
class comb:
    def __init__(self, database_path, XAU_ceof, DXY_coef, TNX_coef, constant):
        """
        传入参数
        database_path ：数据库路径
        constant : 组合价格常数
        ...
        # 建立数据库连接
        conn_db = sqlite3.connect(database_path + "\XAUUSD.db")
        cur_db = conn_db.cursor()

        # 新建组合价格table_list
        cur_db.execute('''CREATE TABLE IF NOT EXISTS comb_list
        ...
        for XAUUSD_data in XAUUSD_data_list:
            ...
            # 组合价格入库
            cur_db.execute("insert into comb_list(DATE, NAME, PRICE, TIMESTICKER, CUR_VOLUME, AGGR_VOLUME) VALUES(?, ?, ?, ?, ?, ?) ",
            ...
            print(str(date) + "\t" + "comb" + "\t" + str(comb_price) + "\t" +
                  str(cur_volume) + "\t" + str(aggr_volume), file=doc)
...
if __name__ == "__main__":
    comb(r"database", 3/7, -3/0.3, -3/0.03, 600)
