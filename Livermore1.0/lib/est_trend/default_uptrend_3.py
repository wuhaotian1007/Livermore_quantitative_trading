...
# 连接数据库data.db和指针
conn = sqlite3.connect('C:\\Users\\sgnjim\\Desktop\\for_python\\Livermore1.0\\database\\XAUUSD_TNX.db')
c = conn.cursor()
...
# 设定第一个值 COLUMN为3
c.execute("update STOCK_LIST SET COLUMN = 3 where ID < 4")
...
DXY_rule = rule("TNX", 2, 0.03/3)
combination_rule = rule("comb", 3, 6/3)
