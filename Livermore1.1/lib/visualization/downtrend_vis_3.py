...
sheet.col(20).width = 9000
sheet.write_merge(0, 0, 1, 7, 'XAUUSD 6.75=3点', style_big)
sheet.write_merge(0, 0, 8, 13, '^TNX 0.03=3点', style_big)
...
for data in data_list:
    ...
    if NAME == "comb":
        if DANGER == 0:
            if TREND == 1:
                sheet.write(row, 20, "上升趋势结束", style_def)
            elif TREND == 2:
                sheet.write(row, 20, "上升趋势恢复", style_def)
            elif TREND == 3:
                sheet.write(row, 20, "下降趋势结束", style_def)
            elif TREND == 6:
                sheet.write(row, 20, "下降趋势恢复", style_def)
            elif TREND == 5:
                sheet.write(row, 20, "上升趋势恢复/下降趋势结束", style_def)
            elif TREND == 7:
                sheet.write(row, 20, "下降趋势恢复/上升趋势结束", style_def)
            elif TREND == 10:
                sheet.write(row, 20, "上升趋势结束(volume不足)", style_def)
            elif TREND == 20:
                sheet.write(row, 20, "上升趋势恢复(volume不足)", style_def)
            elif TREND == 30:
                sheet.write(row, 20, "下降趋势结束(volume不足)", style_def)
            elif TREND == 60:
                sheet.write(row, 20, "下降趋势恢复(volume不足)", style_def)
            elif TREND == 50:
                sheet.write(row, 20, "上升趋势恢复/下降趋势结束(volume不足)", style_def)
            elif TREND == 70:
                sheet.write(row, 20, "下降趋势恢复/上升趋势结束(volume不足)", style_def)
        elif DANGER == 1:
            sheet.write(row, 20, "危险信号", style_def)
        elif DANGER == 10:
            sheet.write(row, 20, "危险信号(volume不足)", style_def)

# 保存文件
xl.save('..\\..\\report\\XAUUSD-TNX-{0}-downtrend.xls'.format(
    time.strftime("%Y-%m-%d")))
