...
# 连接数据库data.db和指针
conn = sqlite3.connect('..\\..\\database\\XAUUSD_DXY_TNX.db')
c = conn.cursor()
        ...
        for i in range(1, length):
            ...
            if name == "comb":
                ...
                if min_ID_down and min_ID_up and min_ID_ndown and min_ID_nup:
                    c.execute(
                        "select price,date,column from STOCK_LIST where NAME = '{0}' and ID = {1}".format('comb', ID))
            ...
            c.execute("select RECORD  from STOCK_LIST where ID = {}".format(ID))
            ...
            if name == "comb":
                c.execute(
                    "select * from STOCK_LIST where TRENDSIGNAL > {0} and ID < {1} ".format(0, ID))
                # 如果存在趋势信号
                if c.fetchall():
                    c.execute(
                        "select price,record,column,date from STOCK_LIST where NAME = '{0}' and ID = {1}".format('comb', ID))
                    info_list = c.fetchall()
                    for info in info_list:
                        # 最新价格信息
                        price = info[0]
                                ...
                                if last_column == 4:
                                    if price <= last_down_price - 3 * coefficient or column * record == 4:
                                        ...
                                        if volume > last_volume * 0.99:
                                            trend_signal += 100
                                            c.execute(
                                                "update STOCK_LIST set TRENDSIGNAL = {0} where ID = {1}".format(trend_signal, ID))
                                        else:
                                            trend_signal += 1000
                                            c.execute(
                                                "update STOCK_LIST set TRENDSIGNAL = {0} where ID = {1}".format(trend_signal, ID))
                                ...
                                if price >= last_down_price + 3 * coefficient:
                                    ...
                                    if volume > last_volume * 0.99:
                                        ...
                                        c.execute(
                                            "update STOCK_LIST set TRENDSIGNAL = {0} where ID = {1}".format(trend_signal, ID))
                        # 当最新一个趋势状态栏为上升趋势结束时
                        elif last_trend_signal in (1, 10, 7, 70):
                            # 上升趋势最新关键点价格若存在
                            c.execute("select price from STOCK_LIST where COLUMN = {0} and KEYSPOT = {1} and name = '{2}' and ID < {3}".format(
                                3, 1, "comb", ID))
                            last_up_price_list = c.fetchall()
