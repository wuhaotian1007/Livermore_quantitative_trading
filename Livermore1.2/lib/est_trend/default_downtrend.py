...
# 连接数据库data.db和指针
conn = sqlite3.connect('C:/Users/sgnjim/Desktop/for_python/Livermore1.2/database/XAUUSD_DXY_TNX.db')
c = conn.cursor()
...
class rule:
    def __init__(self, name, fir_ID, coefficient):
        for i in range(1, length):
            ...
            price = c.fetchone()[0]
            # 时间戳
            c.execute("select timesticker from STOCK_LIST where ID = ? ", (ID,))
            ts = c.fetchone()[0]
