...
class comb:
    def __init__(self, core: str, core_coef: float, database_path: str, constant: float, **argu):
        """
        传入参数
        core : 组合价格核心名称
        core_coef : 核心元素参数
        ...
        **argu : dict [辅助元素名称：辅助元素系数]
        ...
        raw_data_db_path = database_path + r'\raw_data.db'
        # 组合价格辅助元素量quant
        quant = len(argu)
        ...
        # 与原始数据数据库建立连接
        conn2 = sqlite3.connect(raw_data_db_path)
        c2 = conn2.cursor()

        # 与组合价格存储数据库建立连接
        comb_db_path = database_path + r'\{0}.db'.format(f_name)
        conn = sqlite3.connect(comb_db_path)
        c = conn.cursor()
        ...
        for data in core_list:
            ...
            # 将核心数据插入组合价格表中
            c.execute("insert into STOCK_LIST( NAME, DATE, TIMESTICKER, PRICE, VOLUME) \
            ...
            for i in range(quant):
                ...
                # 记录辅助元素
                c.execute("insert into STOCK_LIST( NAME, DATE, TIMESTICKER, PRICE) \
