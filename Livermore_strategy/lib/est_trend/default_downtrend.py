...
class def_down_XAUUSD_DXY_TNX:
    def __init__(self, database_path):
        trading_comb_info_path = database_path + r"\trading_comb_info"
        for file in os.listdir(trading_comb_info_path):
            if "XAUUSD_DXY_TNX.db" in file:
                XAUUSD_DXY_TNX_db_path = trading_comb_info_path + r"/" + file
                # 连接数据库data.db和指针
                ...
                # 建立交易品类数据库连接
                trading_comb_db_path = database_path + r"/trading_comb.db"
                conn_trading_comb = sqlite3.connect(trading_comb_db_path)
                cur_trading_comb = conn_trading_comb.cursor()
                trading_comb_ID = int(file[0])
                cur_trading_comb.execute(
                    "select comb_category from trading_comb_list where trading_comb_ID = {0}".format(trading_comb_ID))
                comb_category = cur_trading_comb.fetchone()[0]
                comb_category_list = comb_category.split(",")

                for i in range(0, len(comb_category_list)):
                    element = comb_category_list[i].split("=")
                    category_name = element[0]
                    category_coefficient = fraction(element[1])
                    rule(category_name, i+1, category_coefficient)

                rule("comb", len(comb_category_list) + 1, 9/3)
...
class def_down_XAUUSD_DXY:
    def __init__(self, database_path):
        trading_comb_info_path = database_path + r"\trading_comb_info"
        for file in os.listdir(trading_comb_info_path):
            if "XAUUSD_DXY.db" in file:
                XAUUSD_DXY_db_path = trading_comb_info_path + r"/" + file
                ...
                rule("comb", len(comb_category_list) + 1, 6/3)
...
class def_down_XAUUSD_TNX:
    def __init__(self, database_path):
        trading_comb_info_path = database_path + r"\trading_comb_info"
        for file in os.listdir(trading_comb_info_path):
            if "XAUUSD_TNX.db" in file:
                XAUUSD_TNX_db_path = trading_comb_info_path + r"/" + file
                # 连接数据库data.db和指针
...
if __name__ == "__main__":
    def_down_XAUUSD_DXY_TNX(r"database")
    def_down_XAUUSD_DXY(r"database")
    def_down_XAUUSD_TNX(r"database")
